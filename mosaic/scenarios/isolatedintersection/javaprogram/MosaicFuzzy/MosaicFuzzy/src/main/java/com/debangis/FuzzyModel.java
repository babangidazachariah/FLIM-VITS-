...
public class FuzzyModel{
	...
	public String EvaluateFIS(double ld, double dt){
		
		StringBuilder status = new StringBuilder();
        if (! engine.isReady(status))
            throw new RuntimeException("[engine error] engine is not ready:\n" + status);

        InputVariable load = engine.getInputVariable("Load");
		InputVariable dirt = engine.getInputVariable("Dirt");
        OutputVariable dtgnt = engine.getOutputVariable("Detergent");
		OutputVariable cle = engine.getOutputVariable("Cycle");
		load.setValue(ld);
		dirt.setValue(dt);
        engine.process();
		return Op.str(dtgnt.getValue()) + Op.str(cle.getValue());
	}
}