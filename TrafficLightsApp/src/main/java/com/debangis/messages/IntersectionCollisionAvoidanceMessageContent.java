...
public class IntersectionCollisionAvoidanceMessageContent implements ToDataOutput, Serializable{
	...
	public IntersectionCollisionAvoidanceMessageContent(String vehid, String pth, long tmeStamp, float lat, float lon, float head, float spd, float acc, float strangle,int vtype) {
	...
	public IntersectionCollisionAvoidanceMessageContent(String vehid, String pth, long tme, float lat, float lon, float head,  float strangle,int vtype) {
		this(vehid, pth, tme, lat, lon, head, 0.0f, 0.0f, strangle, vtype);
	}

	
	public IntersectionCollisionAvoidanceMessageContent(IntersectionCollisionAvoidanceMessageContent icaCnt) {
		this(icaCnt.getVehicleID(),
				icaCnt.getPath(),
				icaCnt.getTimeStamp(),
				icaCnt.getLatitude(),
				icaCnt.getLongitude(),
				icaCnt.getHeading(),
				icaCnt.getSpeed(),
				icaCnt.getAcceleration(),
				icaCnt.getSteeringAngle(),
				icaCnt.getVehicleType()
			);
	}
	public IntersectionCollisionAvoidanceMessageContent(DataInput din) throws IOException{
