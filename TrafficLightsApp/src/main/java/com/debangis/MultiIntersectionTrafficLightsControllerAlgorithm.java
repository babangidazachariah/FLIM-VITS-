...
public class MultiIntersectionTrafficLightsControllerAlgorithm extends AbstractApplication<TrafficLightOperatingSystem> implements CommunicationApplication {
		  ...
		  if (adhocTrafficRecords.rowCount() > 0) {
			  ...
			  if(tmpRecord.rowCount() > 0) {
				  ...
				  if (CompareTrafficStates(previousAdhocTrafficRecords, tmpRecord)) {
					  ...
					  }else if ((impLane1 >= 20) || (impLane2 >= 20)){ // If there is high density on approach between two intersections
	    ...
	    if (msg instanceof SignalRequestMessage) {
	    	...
	    	if (minInterIntersectionTime > 0) { //If true, it means vehicle may pass this intersection
		    	...
		    	if (comingToThisIntersection) {
			    	...
			    	pDuration = (int)getPhaseDuration(wTime, qLength);
			    	int otherVeh = (int)getPhaseDuration(50, ((icnm.getApproachVolume())/3));
	...
	private double getPhaseDuration(double wT, double qL) {
		int pDuration = 0;
		//pDuration = (int) queueDependent.EvaluateFIS(wTime, qLength, 0, 0.0)[0];
    	if (chosenController.equals("fisModel")){
			  double[] fisOutput = fisModel.EvaluateFIS(wT, qL,  (double)maxSpeed, (double)maxDistance);
			  pDuration = (int) fisOutput[0];
			  
		  }else if(chosenController.equals("qdswFisModel")){
			  double[] fisOutput = qdswFisModel.EvaluateFIS(wT, qL,  (double)maxSpeed, (double)maxDistance);
			  pDuration = (int) fisOutput[0];
			  
		  }else if(chosenController.equals("qwdFisModel")){
			  
			  double[] fisOutput = qwdFisModel.EvaluateFIS(wT, qL,  (double)maxSpeed, (double)maxDistance);
			  pDuration = (int) fisOutput[0];
			  
		  }else if(chosenController.equals("bzFisModel")){
			  
			  double[] fisOutput = bzFisModel.EvaluateFIS(wT, qL,  (double)maxSpeed, (double)maxDistance);
			  pDuration = (int) fisOutput[0];
		  }else if(chosenController.equals("queueDependent")){
	  
			  double[] fisOutput = queueDependent.EvaluateFIS(wT, qL,  (double)maxSpeed, (double)maxDistance);
			  pDuration = (int) fisOutput[0];
		  }
    	return pDuration;
	}
