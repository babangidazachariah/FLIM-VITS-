...
public class StaticPhaseTransitPreemptionController extends AbstractApplication<TrafficLightOperatingSystem> implements CommunicationApplication {
			  ...
			  if(tmpRecord.rowCount() > 0) {
				  ...
				  if (nextPhasePriority > 1) { //Meaning high priority vehicle is around this intersection
					...
					//Earliest arriving time
					  ...
					  getConcurrentFlows(appLeg, deptLeg);
				  }else { 
					  roundRobinControl();
				  }
				  
				  nextPhaseDuration = maxPhaseDuration;
			  }else {
				  roundRobinControl();
			  }
