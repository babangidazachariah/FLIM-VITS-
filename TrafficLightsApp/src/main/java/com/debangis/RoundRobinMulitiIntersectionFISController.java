...
public class RoundRobinMulitiIntersectionFISController extends AbstractApplication<TrafficLightOperatingSystem> implements CommunicationApplication {
	...
	private void setNextSchedule() {
				  ...
				  if (CompareTrafficStates(previousAdhocTrafficRecords, tmpRecord)) {
					  ...
					  if (executePriority == false) {
						  ...
						  if ((impLane1 >= 30) || (impLane2 >= 30)){ // If there is high density on approach between two intersections
							  ...
							  chooseNextSchedule();
							  appLeg = nextScheduledApproaches[0];
							  deptLeg = nextScheduledDepartures[0];
	  ...
	  private void chooseNextSchedule() {
		  //This function chooses the next schedule based on roundrobin mechanism with ability to jump a phase if there are no vehicles (queue) in the given phase.
		  roundRobinControl();
		  boolean waitingVehs = false;
		  for (int i = 0; i < nextScheduledApproaches.length; i ++) {
			  int queue = adhocTrafficRecords.where(adhocTrafficRecords.stringColumn("approachleg").isEqualTo(nextScheduledApproaches[i]).and(adhocTrafficRecords.stringColumn("departleg").isEqualTo(nextScheduledDepartures[i]))).rowCount();
			  if (queue >= 1) {
				  waitingVehs = true;
				  break;
			  }
		  }
		  if (waitingVehs == false) {
			  chooseNextSchedule();
		  }
	  }
